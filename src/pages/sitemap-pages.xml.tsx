...
const urlEntry = (
  ...
  `<url><loc>${esc(loc)}</loc><changefreq>monthly</changefreq><priority>${priority}</priority>${hreflangTags(alts, defaultPath)}</url>`;

export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  const locales = ["fr", "en", "es"] as const;

  const pages = [
    { key: "home" as const, priority: "1.0" },
    { key: "about" as const, priority: "0.8" },
    { key: "realisations" as const, priority: "0.8" },
    { key: "contact" as const, priority: "0.7" },
    { key: "blog" as const, priority: "0.7" },
  ];
  ...
  for (const { key, priority } of pages) {
    ...
    for (const entry of entries) {
      urls.push(urlEntry(`${SITE.domain}${entry.path}`, priority, entries, defaultPath));
    }
  }
